...
This package contains some helper functions to specify and analyse univariate and bivariate latent change score (LCS) models using [lavaan](http://lavaan.ugent.be/). 
The two main aims of this package are to 
...
These models can then be connected using the `coupling` argument. 
...
- only one row per estimated parameter `extract_param()`,
- fit statistics for multiple lavaan objects can be looked at using `extract_fit()`.
