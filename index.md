...
- Functions to generate [lavaan](https://lavaan.ugent.be/) syntax for
  ...
  - [`specify_uni_lcsm()`](https://milanwiedemann.github.io/lcsm/reference/specify_uni_lcsm.md):
    Generate syntax for univariate LCSM
  - [`specify_bi_lcsm()`](https://milanwiedemann.github.io/lcsm/reference/specify_bi_lcsm.md):
    Generate syntax for bivariate LCSM
- Functions to fit models using [lavaan](https://lavaan.ugent.be/):
  - [`fit_uni_lcsm()`](https://milanwiedemann.github.io/lcsm/reference/fit_uni_lcsm.md):
    Fit univariate LCSM
  - [`fit_bi_lcsm()`](https://milanwiedemann.github.io/lcsm/reference/fit_bi_lcsm.md):
    Fit bivariate LCSM
...
- Simulate data using [lavaan](https://lavaan.ugent.be/):
  - [`sim_uni_lcsm()`](https://milanwiedemann.github.io/lcsm/reference/sim_uni_lcsm.md):
    Simulate data by specifying parameters for a univariate LCSM
  - [`sim_bi_lcsm()`](https://milanwiedemann.github.io/lcsm/reference/sim_bi_lcsm.md):
    Simulate data by specifying parameters for a bivariate LCSM
- Helper functions:
  - [`plot_lcsm()`](https://milanwiedemann.github.io/lcsm/reference/plot_lcsm.md):
    Visualise LCSM using
    [semPlot](https://github.com/SachaEpskamp/semPlot/)
  - [`select_uni_cases()`](https://milanwiedemann.github.io/lcsm/reference/select_uni_cases.md):
    Select cases for analysis based on available scores on one construct
  - [`select_bi_cases()`](https://milanwiedemann.github.io/lcsm/reference/select_bi_cases.md):
    Select cases for analysis based on available scores on two construct
...
Longitudinal data can be visualised using the
...
and
[`fit_bi_lcsm()`](https://milanwiedemann.github.io/lcsm/reference/fit_bi_lcsm.md)
can be used to fit univariate and bivariate LCSM with different model
specifications. In a first step, these two function generate the user
specified lavaan syntax by calling the
[`specify_uni_lcsm()`](https://milanwiedemann.github.io/lcsm/reference/specify_uni_lcsm.md)
or
[`specify_bi_lcsm()`](https://milanwiedemann.github.io/lcsm/reference/specify_bi_lcsm.md)
functions.
...
help files
[`help(fit_uni_lcsm)`](https://milanwiedemann.github.io/lcsm/reference/fit_uni_lcsm.md).
...
The function
[`fit_uni_lcsm()`](https://milanwiedemann.github.io/lcsm/reference/fit_uni_lcsm.md)
can be used to fit a univariate LCSM using the sample data set
`data_uni_lcsm`. This functions first writes the lavaan syntax specified
in the `model` argument and passes it on to `lavaaan::lavaan()`.
...
the model by the function
[`specify_uni_lcsm()`](https://milanwiedemann.github.io/lcsm/reference/specify_uni_lcsm.md).
The lavaan syntax includes comments describing some parts of the syntax
in more detail. To save the syntax in an object the argument
`return_lavaan_syntax` has to be set to `TRUE`. This object looks a bit
funny, it’s one very long line of text, but can be formatted to look
more beautiful and readable using `cat(syntax)`.
...
The function
[`fit_bi_lcsm()`](https://milanwiedemann.github.io/lcsm/reference/fit_bi_lcsm.md)
allows to specify two univariate LCSMs using the arguments `model_x` and
`model_x`. These two constructs can then be connected using the
`coupling` argument. More details can be found in the help files
[`help(fit_bi_lcsm)`](https://milanwiedemann.github.io/lcsm/reference/fit_bi_lcsm.md).
...
bivariate LCSMs that were specified with
[`fit_uni_lcsm()`](https://milanwiedemann.github.io/lcsm/reference/fit_uni_lcsm.md)
or
[`fit_bi_lcsm()`](https://milanwiedemann.github.io/lcsm/reference/fit_bi_lcsm.md).
Modified LCSMs will probably return errors as the layout matrix that
gets created by this plot function only supports the specifications that
can be modelled with this package. The input arguments for plotting a
simplified path diagram are:
...
and
[`sim_bi_lcsm()`](https://milanwiedemann.github.io/lcsm/reference/sim_bi_lcsm.md)
simulate data based on some some parameters that can be specified. See
the tables [here](#overview-of-estimated-lcs-model-parameters) for a
